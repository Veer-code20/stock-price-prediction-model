...
describe('Market workspace acceptance', () => {
  ...
  it('persists watchlist changes and clears a forecast when the horizon changes', async () => {
    ...
    fireEvent.click(screen.getByRole('button',{name:'Estimate price'}));
    expect(await screen.findByText('AAPL · about 1 month')).toBeInTheDocument();
    fireEvent.change(screen.getByLabelText('Timeframe'),{target:{value:'5'}});
    expect(screen.queryByText('AAPL · about 1 month')).not.toBeInTheDocument();
  });
  it('discards a superseded forecast after switching stocks', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('button',{name:'Estimate price'})).toBeEnabled());
    expect(screen.queryByText('AAPL · about 1 month')).not.toBeInTheDocument();
  });
