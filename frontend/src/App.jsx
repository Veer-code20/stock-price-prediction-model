...
export default function App() {
  ...
  const visible = enriched.filter(stock => `${stock.symbol} ${stock.name}`.toLowerCase().includes(query.trim().toLowerCase()) && (tab !== 'watchlist' || watchlist.includes(stock.symbol)));
      ...
      <div className="content">
        ...
        <div className="workspace-grid">
          ...
          <div className="detail-column">
            ...
            <section className="panel forecast-panel"><div className="forecast-title"><span className="forecast-icon"><ChartNoAxesCombined size={21}/></span><div><h2>Price scenario</h2><p>A simple trend projection from price history.</p></div><span className="outline-pill">EXPERIMENTAL</span></div>
              <div className="forecast-controls"><div><label htmlFor="horizon">Forecast horizon</label><select id="horizon" value={horizon} onChange={event => {resetForecast(); setHorizon(Number(event.target.value));}}><option value={5}>5 sessions · short term</option><option value={20}>20 sessions · one month</option><option value={60}>60 sessions · three months</option></select></div><button className="primary" onClick={generateForecast} disabled={!detail || forecastStatus === 'loading'}>{forecastStatus === 'loading' ? 'Generating…' : 'Explore forecast'}<ArrowRight size={17}/></button></div>
              <div aria-live="polite">{forecastError && <Failure text={forecastError} retry={generateForecast}/>} {forecast && <div className="forecast-result"><div><span>{forecast.symbol} · {forecast.horizon} sessions</span><strong>{money(forecast.projected_price)}</strong><small>Illustrative endpoint</small></div><div><span>Illustrative range</span><strong className="bounds">{money(last.lower)}–{money(last.upper)}</strong><small>Not a calibrated confidence interval</small></div></div>}</div>
              <div className="forecast-note"><FlaskConical size={15}/><p>Illustrative only. Uses {detail?.mode === 'demo' ? 'synthetic' : 'historical'} prices and a simple trend, not a trained prediction model.</p></div>
            </section>
          </div>
        </div>
